...
volatile int baseBuffer = 0;
volatile int tiltBuffer = 0;
volatile int baseSetPoint = 0;
volatile int tiltSetPoint = 0;

volatile int newLineCount;
...
int main(void)
{
  ...
  PID(&basePID, &posIn1, &spOut1, &goal1, 0.9, 0.4, 0.03, _PID_P_ON_E, _PID_CD_DIRECT);
  PID(&tiltPID, &posIn2, &spOut2, &goal2, 0.6, 0.1, 0.1, _PID_P_ON_E, _PID_CD_DIRECT);

  PID_SetMode(&basePID, _PID_MODE_AUTOMATIC);
  PID_SetSampleTime(&basePID, 500);
  PID_SetOutputLimits(&basePID, 0.03, 1);

  PID_SetMode(&tiltPID, _PID_MODE_AUTOMATIC);
  PID_SetSampleTime(&tiltPID, 500);
  PID_SetOutputLimits(&tiltPID, 0.01, 1);
  /* USER CODE END Init */
  ...
  while (1)
  {
	  ...
	  sprintf(MSGsp1, "Base Set Point = %d ", baseSetPoint);
	  sprintf(MSGsp2, "Tilt Set Point = %d ", tiltSetPoint);

	  sprintf(MSGsp3, "Base PID Out = %d ", (int)(spOut1*65535));
	  ...
	  HAL_UART_Transmit(&huart3, MSGsp3, sizeof(MSGsp1), 100);
	  HAL_UART_Transmit(&huart3, MSGsp4, sizeof(MSGsp2), 100);

	  HAL_UART_Receive_IT(&huart3, &RX_Char, 1);

	  runMotors(&basePID, &tiltPID, baseSetPoint, tiltSetPoint);
    /* USER CODE END WHILE */
